...
import { getBlogsPopular, getBlogsFollowersFollowings } from '../../Actions/blogAction';
import BlogCard from './BlogCard';

const FollowUnfollowBlogs = () => {
    const navigation = useNavigation();
    const dispatch = useDispatch();
    const contentWidth = Dimensions.get('window').width;
    const {blogsFollowers: blogs, size: blogSize} = useSelector((state) => state.blog);
    ...
    const getBlogsFollowersHandler = async () => {
      dispatch(getBlogsFollowersFollowings(skip, limit));
  ...
  const loadMoreBlogs = async () => {
        ...
        const {data} = await axios.post(`${API}/blog/blogs-followers-followings`, {skip: toSkip, limit, token}, config);
...
export default FollowUnfollowBlogs
