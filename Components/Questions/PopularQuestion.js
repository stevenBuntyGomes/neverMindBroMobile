...
import { getQiuestionPopular } from '../../Actions/questionAnswerAction';
...
const PopularQuestion = () => {
  ...
  const {
    message, 
    questionsPopular, 
    questionPopularSize: questionSize, 
  ...
  const followUnfollowQuestionHandler = async (questionId) => {
    await dispatch(getQiuestionPopular(questionId));
  }
  // follow unfollow question handler
...
//   }
  ...
  const getQuestionHandler = async () => {
    await dispatch(getQiuestionPopular(skip, limit));
    setMainQuestions([...questionsPopular]);
  ...
  const loadMoreQuestionHandler = async () => {
    ...
    const {data, status} = await axios.post(`${API}/question/get-question-user`, {toSkip, limit}, config);
...
const styles = StyleSheet.create({
  ...
  individualQuestionContainer: {
    ...
    borderRadius: 8,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 0 }, // Slight negative width to remove left/right shadow
    shadowOpacity: 0.1,
    shadowRadius: 8,
    elevation: 1,
  },
