...
import { getQuestionsCategoriesTags } from '../../Actions/questionAnswerAction';
...
const TagsCategories = () => {
  ...
  //   await dispatch(getQuestionsCategoriesTags(questionId));
  // }
  // follow unfollow question handler
...
//   }
  ...
  const getQuestionTagsHandler = async () => {
    // setSelectedTags(selectedItems);
    await dispatch(getQuestionsCategoriesTags(skip, limit, selectedCategories, selectedTags)); 
    setMainQuestions([...questionsTagsCategories]);
  ...
  const loadMoreQuestionHandler = async () => {
    ...
    const {data, status} = await axios.post(`${API}/question/get-question-categories-tags`, {toSkip, limit, selectedCategories, selectedTags}, config);
  ...
  useEffect(() => {
    getQuestionTagsHandler();
    setMainQuestions([...questionsTagsCategories]);
    setSize(questionSize);
  }, [selectedTags, questionSize]);
